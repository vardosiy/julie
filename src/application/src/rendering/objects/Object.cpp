...
#include "rendering/common/Axis.hpp"
...
void Object::setParameters(const Parameters & _params) noexcept
{
	m_params = _params;
}
...
void Object::setUniforms()
{
	...
	Camera & cam = sceneMgr.getCamera();

	const Uniforms & uniforms = m_shader->getUniforms();

	if (uniforms.u_WVP != -1)
		m_shader->setUniformValue(uniforms.u_WVP, getWorldMatrix() * cam.getViewProjectionMatrix());
	...
	if (m_bIsLighted)
	{
		const AmbientLight & ambientLight = sceneMgr.getAmbientLight();

		m_shader->setUniformValue(uniforms.u_ambientColor, ambientLight.getColor());
		m_shader->setUniformValue(uniforms.u_ambientWeight, ambientLight.GetWeight());
	...
	if (m_bIsFogged)
	{
		const Fog & fog = sceneMgr.getFog();

		m_shader->setUniformValue(uniforms.u_fogStart, fog.getStart());
		m_shader->setUniformValue(uniforms.u_fogRange, fog.getRange());
		m_shader->setUniformValue(uniforms.u_fogColor, fog.getColor());
	}
}
...
glm::mat4 & Object::getWorldMatrix()
{
	if (m_bIsModified)
	{
		glm::mat4 scale = glm::scale(glm::mat4(), glm::vec3(m_scale));

		// division required for making translation independent from scale factor
		glm::mat4 translate = glm::translate(glm::mat4(), glm::vec3(m_pos.x / m_scale.x, m_pos.y / m_scale.y, m_pos.z / m_scale.z));

		glm::mat4 rotateX = glm::rotate(glm::mat4(), m_rotation.x, constants::axis::x);
		glm::mat4 rotateY = glm::rotate(glm::mat4(), m_rotation.y, constants::axis::y);
		glm::mat4 rotateZ = glm::rotate(glm::mat4(), m_rotation.z, constants::axis::z);
