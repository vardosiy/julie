...
static LightType lightTypeFromString(std::string_view _str)
{
	if (_str == "directional")
	{
...
void SceneManager::loadAmbientLight(const Json::Value & _data)
{
	...
	const glm::vec4 color{ details::jsonArrayToVec3(_data["color"]), 1.0f };
	const float weight{ _data["weight"].asFloat() };

	m_ambientLight = std::make_unique<AmbientLight>(color, weight);
}
...
void SceneManager::loadFog(const Json::Value & _data)
{
	...
	const glm::vec4 color{ details::jsonArrayToVec3(_data["color"]), 0.0f };
	const float start{ _data["start"].asFloat() };
	const float range{ _data["range"].asFloat() };

	m_fog = std::make_unique<Fog>(start, range, color);
}
...
void SceneManager::loadLights(const Json::Value & _data)
{
	...
	for (uint32_t i{ 0 }; i < lightsCount; ++i)
	{
		...
		LightType lightType = details::lightTypeFromString(current["type"].asString());
...
Parameters SceneManager::loadObjectParameters(const Json::Value & _data)
{
	Parameters params;
...
)
{
	...
	std::vector<T *> textures(texturesCount);
